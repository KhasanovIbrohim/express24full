...
const handleClick = () => {
setUpdated(message);
localStorage.setItem('phone', JSON.stringify(message));
};
return (
<nav className="navbar">
  <div className="nav">
    <div className="nav-logo"><a href="/"><span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 681.87 194.46">
            <path className="cls-1" d="M221.85,200.61a97.23,97.23,0,1,1-97.23,97.23A97.28,97.28,0,0,1,221.85,200.61Z"
    ...
    <div className="nav-search" data-fluid="false" data-error="false">
      <svg className='nav-search-icon' width="24" height="24" viewBox="0 0 24 24" fill="none"
        ...
        <path
          ...
          fill="transparent" stroke="#1A1A18" strokeLinecap="round" strokeLinejoin="round"></path>
        <path d="M20 20L17 17" stroke="#1A1A18" strokeLinecap="round" strokeLinejoin="round"></path>
      </svg>
      <input type="text" required="" className="nav-search-input" placeholder="Заведение, блюдо, товар или кухня"
        data-empty="true" data-prefix="true" data-error="false" data-static="true" />
      <button className='nav-search-button'>Найти</button>
    </div>
    <div className="nav-banner" onClick={()=> setShowModal(true)}>
      <svg className='nav-banner-icon' width="24" height="24" viewBox="0 0 24 24" fill="none"
        xmlns="http://www.w3.org/2000/svg">
      ...
      <h1 className='nav-banner-title'>Мирабад район, Амира Темура проспект</h1>
    </div>
  </div>
  <div ref={userModal} className="nav-profile" onClick={()=> setShowModal2(true)}>
    <svg className='nav-profile-icon' width="24" height="24" viewBox="0 0 24 24" fill="none"
      xmlns="http://www.w3.org/2000/svg">
      <path
        ...
        stroke="#1A1A18" strokeLinecap="round" strokeLinejoin="round"></path>
      <path
    ...
    <p className='nav-profile-title'>Профиль</p>
  </div>
  <Modal className="modal" show={showModal} onHide={handleClose} renderBackdrop={renderBackdrop}>
    <div>
      ...
      <Iframe className='modal-location' src="https://yandex.uz/map-widget/v1/-/CCtA5~r" frameborder="1"
      ...
      <form>
        ...
        <input type="text" placeholder='Комментарий (код домофона, ориентир)' />
        <button className='modal-submit'>Подтвердить</button>
      </form>
    </div>
  </Modal>
  <Modal className="modalLogin" show={showModal2} onHide={handleClose2} renderBackdrop={renderBackdrop2}>
    <div>
      <div>
        <h1>Введите номер телефона <br /> <span>и получите код подтверждения</span></h1>
        <button className='modalLogin-button' onClick={handleClose2}>X</button>
      </div>
      <form>
        <input onChange={handleChange} type="text" placeholder='Номер телефона' />
        <p>Нажимая кнопку получить код вы принимаете условия <br /><a href="/">пользовательского соглашения</a></p>
